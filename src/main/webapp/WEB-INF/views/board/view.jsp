...
<body>
					...
					<div class="reply">
						...
						<c:if test="${member.isAdmin == 1}">
							<form id="deleteForm" action="/board/deletereply" method="post">
								...
								<input type="hidden" readonly="readonly" name="memberId"
		...
		<c:if test="${member.isAdmin == 1}">
			<form action="/board/reply" method="post">
				<input type="hidden" id="boardId" name="boardId"
					value="${board.boardId}" /> <label for="writer"><fmt:message
						key="WRITER" />: </label> <input type="text" readonly="readonly"
					name="memberId" value="${member.memberId}" /> <br /> <label
					for="content"></label>
	...
	<c:if
		test="${board.memberId == sessionScope.memberId || member.isAdmin==1}">
		<div class="boardBtns">
			<a class="updateBtn"
				href='<c:url value="/board/update/${board.boardId}"/>'><button
					type="button" class="genric-btn info">
					<fmt:message key="UPDATE" />
				</button></a> <input type="hidden" id="memberId" readonly="readonly"
				name="memberId" value="${member.memberId}" /> <input type="hidden"
				id="boardId" name="boardId" value="${board.boardId}" /> <a
				class="deleteBtn"><button type="button" id="deleteBoard"
